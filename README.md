...
> 状态：**异网组网核心链路双机实测打通**（v0.2.6，2026-09-01）：BLE 发现→握手→GATT 持久信令→异网判定→角色仲裁→④ 手动开热点→offer 经 BLE 发从机→从机自动 Specifier 接入→joined 回报→组网成 LAN。一期握手 v0.1.7；组网 A 包 v0.2.x 系列（含写队列/对端接管/权限矩阵修复）。
