...
> 2026-09-03 快照（正式发布 v1.0.0（v1.0.0 时点：组网+LocalSend 闭环 ✅ / UI 经 MD3 重构至扁平分层 / 广播钮呼吸定版 / 个性化页 UI1b-B 三槽壁纸 + UI1b-B2 重做（一屏无滚动布局+1:5 取色+右上保存+主页面浮层化+强调色主题）✅ / 品牌蓝鲸·X+图标 / GPL-3.0 开源 / GitHub 远端 oh-zgl-s-fish/Bluelink）。配套：README（路线）、docs/networking.md（组网设计）、docs/troubleshooting.md（排障）。
...
- 个性化「容器透明度」可读性提醒**预留固定留白位**：透明度行下方常驻固定高度留白区（高度≈提醒文字），≥40% 风险提醒原地填入该区——提醒出现/消失**不引起任何元素位置变动**（避免当前动态插入导致的布局位移）
