...
- 双方**同时**都想开（冲突）→ 自动裁定，优先级：**root > 私有 API 可行 > 电量高**（相同级别时电量高者开）
...
| ② | **私有 API 常规热点**（反射 `setWifiApEnabled`，MacroDroid 式） | 非 root 且反射可行（按版本 try 实测；8–9 可靠，10–12 机型差异，13+ 大概率不可） | 自设 → 自动发 |
| ③ | **LocalOnlyHotspot**（公开 API，8.0+） | ①②不可用 | 8–9 自动读 `getWifiConfiguration()`；13+ 系统弹窗展示→用户回填；10–12 读不到→引导手动开系统热点回填 |

- 密码盲区兜底（10–12 非 root）：提示用户**手动开常规系统热点**（自己设的密码）→ 填进 App → 经 BLE 发对端（一次人工参与全版本可行）
- SSID/密码策略：App 自设固定格式（`Bluelink-XXXX` + 随机 8 位密码），对端自动连最顺
...
IDLE ──用户确认──▶ NEGOTIATING(角色裁定/可切换)
   ──▶ HOTSPOT_STARTING(三级策略逐级尝试,每级失败降级)
...
2. 12+ 测试机：私有 API 可行性（预期被拦 → 走 LocalOnlyHotspot + 手动回填路径）
3. LocalOnlyHotspot 各版本密码获取实测（8-9 读 / 13+ 弹窗）